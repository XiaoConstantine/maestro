...
- **Unified Review Component**: inline `/review` results and standalone `RunReviewTUI` share file grouping, flattened selection, severity filters, list/detail rendering, and colorblind-safe severity shapes. Standalone posting confirmation remains available when its host supplies a posting callback; host-specific quit/status semantics stay at the wrapper boundary.
- **Accessible Presentation**: `--high-contrast` or `MAESTRO_THEME=high-contrast` increases text and border contrast; `--reduce-motion` or `MAESTRO_REDUCE_MOTION=1` replaces animated status frames with a static label. Standard `NO_COLOR` disables color without enabling reduced motion.
- **Responsive Agent Context**: the header identifies the authoritative workspace and active model. At 80+ columns, active run/review context can move into an adaptive rail; narrow panes retain a single chronological transcript. Ctrl+\\ toggles the rail without discarding its state.
- **Adaptive Composer**: multiline prompts grow the editor to at most 30% of the terminal or 10 rows and show line/character counts while preserving a usable conversation region.
