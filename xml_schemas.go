...
// XMLConfigFactory creates appropriate XML configurations for different tool types.
type XMLConfigFactory struct {
...
// NewXMLConfigFactory creates a new XML configuration factory.
func NewXMLConfigFactory() *XMLConfigFactory {
	return &XMLConfigFactory{
		successRates:  make(map[string]float64),
		avgParseTimes: make(map[string]time.Duration),
	}
}

// GetSearchToolXMLConfig returns XML configuration optimized for search tools.
func (x *XMLConfigFactory) GetSearchToolXMLConfig() interceptors.XMLConfig {
...
// GetFileToolXMLConfig returns XML configuration optimized for file tools.
func (x *XMLConfigFactory) GetFileToolXMLConfig() interceptors.XMLConfig {
	return interceptors.XMLConfig{
...
// GetGeneralXMLConfig returns XML configuration for general tool usage.
func (x *XMLConfigFactory) GetGeneralXMLConfig() interceptors.XMLConfig {
	return interceptors.XMLConfig{
...
// GetAdaptiveXMLConfig returns dynamically optimized XML configuration based on performance metrics.
func (x *XMLConfigFactory) GetAdaptiveXMLConfig(toolName string, queryComplexity int) interceptors.XMLConfig {
	x.mu.RLock()
...
func (x *XMLConfigFactory) RecordPerformance(toolName string, success bool, parseTime time.Duration) {
	x.mu.Lock()
	defer x.mu.Unlock()
