// Package metrics provides metrics collection and analysis for code reviews.
package metrics
...
// FeedbackStats tracks user feedback.
...
// ThreadMetrics tracks metrics for a review thread.
...
// ReviewMetricsCollector interface for extended metrics collection.
type ReviewMetricsCollector interface {
	// Session tracking
	StartReviewSession(ctx context.Context, prNumber int)
	TrackNewThread(ctx context.Context, threadID int64, comment types.PRReviewComment)

	// Comment impact tracking
	TrackCommentModification(ctx context.Context, comment *github.PullRequestComment)
	TrackHistoricalComment(ctx context.Context, comment types.PRReviewComment)
	...
	GenerateReport(ctx context.Context) *types.BusinessReport
}
...
type BusinessMetrics struct {
	...
	categoryMetrics map[string]*types.CategoryStats
...
func NewBusinessMetrics(logger *logging.Logger) types.MetricsCollector {
	return &BusinessMetrics{
		logger:            logger,
		categoryMetrics:   make(map[string]*types.CategoryStats),
...
func (bm *BusinessMetrics) TrackNewThread(ctx context.Context, threadID int64, comment types.PRReviewComment) {
	bm.mu.Lock()
	defer bm.mu.Unlock()
	...
	if stats, exists := bm.categoryMetrics[comment.Category]; exists {
		...
		bm.categoryMetrics[comment.Category] = &types.CategoryStats{
			TotalComments: 1,
...
func (bm *BusinessMetrics) TrackHistoricalComment(ctx context.Context, comment types.PRReviewComment) {
	bm.mu.Lock()
	defer bm.mu.Unlock()
...
func (bm *BusinessMetrics) TrackReviewComment(ctx context.Context, comment types.PRReviewComment, isValid bool) {
	bm.mu.Lock()
	defer bm.mu.Unlock()
...
func (bm *BusinessMetrics) TrackCommentResolution(ctx context.Context, threadID int64, resolution types.ResolutionOutcome) {
	bm.mu.Lock()
	defer bm.mu.Unlock()
	...
	thread.IsResolved = resolution == types.ResolutionAccepted
...
func (bm *BusinessMetrics) GenerateReport(ctx context.Context) *types.BusinessReport {
	bm.mu.RLock()
	defer bm.mu.RUnlock()

	report := &types.BusinessReport{
		GeneratedAt:       time.Now(),
		CategoryMetrics:   make(map[string]types.CategoryMetrics),
	...
	for category, stats := range bm.categoryMetrics {
		...
		report.CategoryMetrics[category] = types.CategoryMetrics{
			Precision:     precision,
...
func (bm *BusinessMetrics) GetCategoryMetrics(category string) *types.CategoryStats {
	bm.mu.RLock()
	defer bm.mu.RUnlock()
...
// StartThreadTracking starts tracking for a thread.
func (bm *BusinessMetrics) StartThreadTracking(ctx context.Context, comment types.PRReviewComment) {
...
func (bm *BusinessMetrics) storeOptimizationResults(ctx context.Context, trends map[string]*TrendAnalysis, report *types.BusinessReport) error {
	bm.mu.Lock()
	defer bm.mu.Unlock()
