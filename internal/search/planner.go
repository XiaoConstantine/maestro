...
// Planner analyzes queries and creates search strategies.
type Planner struct {
	llm    core.LLM
	logger *logging.Logger
}

// Router routes queries to appropriate search strategies.
type Router struct {
	planner *Planner
	rules   []*RoutingRule
	logger  *logging.Logger
...
// NewPlanner creates a new search planner.
...
func (sp *Planner) CreateSearchPlan(ctx context.Context, query string, codeContext string) (*Plan, error) {
...
func (sp *Planner) analyzeIntent(ctx context.Context, query string, codeContext string) (*Intent, error) {
...
// parseIntentResponse parses the LLM response into Intent.
func (sp *Planner) parseIntentResponse(response string) *Intent {
	intent := &Intent{
...
func (sp *Planner) createSpawnStrategy(intent *Intent) *SpawnStrategy {
	strategy := &SpawnStrategy{
		MaxParallel:      3, // default
		TokensPerAgent:   100000,
		TypeDistribution: make(map[types.SearchAgentType]float64),
	...
	strategy.TypeDistribution[types.CodeSearchAgent] = 1.0

	return strategy
}

// generateSearchSteps creates a unified search plan for the agent pool.
func (sp *Planner) generateSearchSteps(intent *Intent, strategy *SpawnStrategy) []*Step {
	var steps []*Step
...
func (sp *Planner) estimateStepDuration(agentType SearchAgentType, complexity int) time.Duration {
...
func (sp *Planner) estimateDuration(steps []*Step) time.Duration {
...
func (sp *Planner) defineSuccessMetrics(intent *Intent) *SuccessMetrics {
...
func (sp *Planner) extractKeywords(query string) []string {
...
func (sp *Planner) extractFileHints(query string) []string {
	var hints []string
...
func (sp *Planner) generateUnifiedDescription(intent *Intent) string {
	return fmt.Sprintf("Unified search for %s intent with %s scope (complexity: %d)",
		intent.Primary, intent.Scope, intent.Complexity)
}

// createUnifiedSearchRequest receives the original query and context from the calling method.
func (sp *Planner) createUnifiedSearchRequest(intent *Intent) *SearchRequest {
...
// NewRouter creates a new search router with default rules.
func NewRouter(planner *Planner, logger *logging.Logger) *Router {
	router := &Router{
		planner: planner,
		logger:  logger,
...
func (sr *Router) addDefaultRules() {
...
func (sr *Router) Route(ctx context.Context, query string) (*Plan, error) {
...
func (sr *Router) AddRule(rule *RoutingRule) {
	sr.rules = append(sr.rules, rule)
}

// Utility functions

// ExtractNumber extracts a single digit number from a line.
