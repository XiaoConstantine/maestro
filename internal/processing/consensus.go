// Package processing provides processing logic for code review operations.
package processing
...
type IssueValidationInput struct {
	Issue       types.ReviewIssue `json:"issue"`
...
type SingleValidationResult struct {
	...
	ValidatorResults    []types.ValidatorResult `json:"validator_results"`
}

// NewConsensusValidationProcessor creates a new consensus validation processor.
func NewConsensusValidationProcessor(metrics types.MetricsCollector, logger *logging.Logger) *ConsensusValidationProcessor {
...
func (p *ConsensusValidationProcessor) Process(ctx context.Context, task agents.Task, taskContext map[string]interface{}) (interface{}, error) {
	...
	var validatedIssues []types.ReviewIssue
	var rejectedIssues []types.ReviewIssue
	var validatorResults []types.ValidatorResult
	...
	result := &types.ConsensusValidationResult{
...
func (p *ConsensusValidationProcessor) parseValidatorPerspectives(perspectivesData interface{}) []types.ValidatorResult {
	var results []types.ValidatorResult
...
func (p *ConsensusValidationProcessor) extractValidationInput(task agents.Task, taskContext map[string]interface{}) ([]IssueValidationInput, error) {
	...
	if enhancedResult, ok := task.Metadata["enhanced_result"].(*types.EnhancedReviewResult); ok {
	...
	codeContext := util.GetStringFromContext(taskContext, "code_context", "")
	guidelines := util.GetStringFromContext(taskContext, "guidelines", "Follow standard coding practices")
	repoContext := util.GetStringFromContext(taskContext, "repository_context", "")
...
func (p *ConsensusValidationProcessor) calculateOverallConsensusScore(results []types.ValidatorResult) float64 {
	if len(results) == 0 {
		return 0.5
	}
...
func (p *ConsensusValidationProcessor) fallbackToSingleValidation(ctx context.Context, task agents.Task, taskContext map[string]interface{}) (interface{}, error) {
	...
	return &types.ConsensusValidationResult{
		ValidatedIssues: validatedIssues,
		RejectedIssues:  []types.ReviewIssue{},
...
func (p *ConsensusValidationProcessor) trackValidationMetrics(ctx context.Context, result *types.ConsensusValidationResult) {
	_ = p.metrics // Metrics tracking not yet implemented
}

// IsConsensusValidationEnabled checks if consensus validation is enabled.
func IsConsensusValidationEnabled() bool {
	return util.GetEnvBool("MAESTRO_CONSENSUS_VALIDATION", true)
}
