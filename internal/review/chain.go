...
// ReviewChainOutput is defined in types_aliases.go
...
func (p *ReviewChainProcessor) parseWorkflowResults(workflowResult map[string]interface{}, output *ReviewChainOutput) error {
			...
			for _, issue := range issues {
				if issueMap, ok := issue.(map[string]interface{}); ok {
					...
					potentialIssue := PotentialIssue{
						FilePath:   util.SafeGetString(issueMap, "file_path"),
						LineNumber: util.SafeGetInt(issueMap, "line_number"),
						RuleID:     util.SafeGetString(issueMap, "rule_id"),
						Content:    util.SafeGetString(issueMap, "content"),
						Category:   util.SafeGetString(issueMap, "category"),
						Suggestion: util.SafeGetString(issueMap, "suggestion"),
						Confidence: util.SafeGetFloat(issueMap, "confidence"),
					}
					...
					if contextMap, ok := issueMap["context"].(map[string]interface{}); ok {
						potentialIssue.Context = map[string]string{
							"before": util.SafeGetString(contextMap, "before"),
							"after":  util.SafeGetString(contextMap, "after"),
						}
					}
	...
	if contextResult, ok := workflowResult["context_validation"].(map[string]interface{}); ok {
		output.ContextValidation.Valid = util.SafeGetBool(contextResult, "context_valid")
		output.ContextValidation.Confidence = util.SafeGetFloat(contextResult, "confidence")
		output.ContextValidation.EnhancedContext = util.SafeGetString(contextResult, "enhanced_context")
	}
	...
	if ruleCompResult, ok := workflowResult["rule_compliance"].(map[string]interface{}); ok {
		output.RuleCompliance.Compliant = util.SafeGetBool(ruleCompResult, "rule_compliant")
		output.RuleCompliance.RefinedSuggestion = util.SafeGetString(ruleCompResult, "refined_suggestion")
	}
	...
	if impactResult, ok := workflowResult["practical_impact"].(map[string]interface{}); ok {
		output.PracticalImpact.IsActionable = util.SafeGetBool(impactResult, "is_actionable")
		output.PracticalImpact.FinalSuggestion = util.SafeGetString(impactResult, "final_suggestion")
		output.PracticalImpact.Severity = util.SafeGetString(impactResult, "severity")
	}

	// Parse review metadata - captures file and thread information
	if metadata, ok := workflowResult["metadata"].(map[string]interface{}); ok {
		output.ReviewMetadata.FilePath = util.SafeGetString(metadata, "file_path")
		output.ReviewMetadata.Category = util.SafeGetString(metadata, "category")
		...
		if lineRange, ok := metadata["line_range"].(map[string]interface{}); ok {
			output.ReviewMetadata.LineRange = LineRange{
				Start: util.SafeGetInt(lineRange, "start"),
				End:   util.SafeGetInt(lineRange, "end"),
				File:  util.SafeGetString(lineRange, "file"),
			}
		}
