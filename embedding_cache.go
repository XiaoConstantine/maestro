...
func (ec *EmbeddingCache) cacheKey(input string) string {
	hash := sha256.Sum256([]byte(input))
	return fmt.Sprintf("%x", hash)
}

// Get retrieves a cached embedding result.
func (ec *EmbeddingCache) Get(input string) *core.EmbeddingResult {
	ec.mu.RLock()
	defer ec.mu.RUnlock()
...
func (ec *EmbeddingCache) GetWithMetadata(input string) (*core.EmbeddingResult, *CachedEmbedding) {
	ec.mu.RLock()
	defer ec.mu.RUnlock()
...
func (ec *EmbeddingCache) Set(input string, result *core.EmbeddingResult, source string) {
	ec.mu.Lock()
	defer ec.mu.Unlock()
